...
const COMMIT_MESSAGE_REGEXP = new RegExp(
  `^(((${ProjectPrefix.APP})-[0-9]{1,6})|(${ProjectPrefix.ENVIRONMENTS.join('|')})): ([${COMMIT_MODIFIERS.join(',')}]) (.*\\S)$`,
);
...
const configuration: UserConfig = {
  ...
  plugins: [
    {
      rules: {
        'commit-message-match': ({ header }): [boolean, string] => {
          if (!header || !COMMIT_MESSAGE_REGEXP.test(header)) {
